...
describe("実プロセス起動(AC8): tag-version をネットワーク不要で実行する", () => {
  ...
  it("前提: packages/app/package.json の version が 1.6.6 である", () => {
    expect(appPkg.version).toBe("1.6.6");
  });
  ...
  it("バージョン一致(v1.6.6)で終了コード0", () => {
    const result = spawnSync(
      process.execPath,
      ["--import", "tsx", SCRIPT_ABS_PATH, "tag-version", "v1.6.6"],
      { encoding: "utf8" },
    );
