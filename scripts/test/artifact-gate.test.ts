 ...
 * ABI不一致(NODE_MODULE_VERSION不一致等)を1件も検出できないことが**対照実験**で確認
 * されている(Node 向け〈NODE_MODULE_VERSION=127〉の `.node` を Electron 向け配置へ注入すると、
 * 配置検査4項目〈当初案〉はすべて PASS した一方、ヘッドレススモークは
 * `NODE_MODULE_VERSION 127 ... requires 132` で実際に FAIL した)。**v1.3.1 の実物 exe は
 * この対照実験の対象ではない**(実物の `.node` は `nm_version=132` で Electron 向けに正しく
 * ビルド済みであり、配置検査・スモークとも実際には allow を返す。詳細は
 * `scripts/artifact-gate.ts` 冒頭「設計の核心」参照)。そのため本テストは
