 ...
 * - **配分層(任意)**: 本スクリプトでは出していない。配分結果(Σx*・betCount・totalStake)を
 *   テストの期待値にすると、同じ数値が2箇所で管理される二重定義になるため
 *   (#20-A着手前ゲート第2回の裁定)。#20-Bは#80(#78-A)/#81(#78-B)に分割され、既定モデルの
 *   切替は#81で完了済み——配分層スイープの`scripts/`追加自体は#81のスコープ外
 *   (#81着手前ゲート裁定:「やるなら別Issue」)であり、着手する場合は改めて別Issueを立てる。
