 ...
 * 「**両 CONFIG 間の参照先の取り違え**」の2種であって、`bet_unit`/`greedy_steps`/`candidate_cap`
 * の**列どうしの入れ替え**は現に検出できる。ただしそれは3つの既定値が 100 / 1000 / 2000 と
 * **互いに異なる値だから**であって、この保護は値が異なることに依存した偶然のものである。
 * 将来このうち2つが同値になれば、入れ替えの検出は**テストが緑のまま静かに消える**
 * (まさに本ファイルが防ごうとしている「保存済みの列が静かに嘘になる」のと同型の劣化が、
 * この JSDoc の記述自体に起きる)。既定値を変えるときはここを読み直すこと。
 */
