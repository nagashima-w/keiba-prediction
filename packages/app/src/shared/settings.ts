...
export const INCLUDE_COMBO_ODDS_LABELS = {
  /** 設定画面のチェックボックスのラベル。 */
  checkbox: "ワイド・馬連・三連複のオッズも取得する(上級)",
  /** チェックボックス直下の補助文。 */
  help:
    "取得に時間がかかります(地方競馬の三連複は1レースあたり最大16リクエスト・約24秒。馬連は1レースあたり常に1リクエスト追加。12レース一括で+約5分)。取得したオッズを馬券配分に使うかどうかは、下記「ワイドを配分に使う」「馬連を配分に使う」「三連複を配分に使う」の設定に従います。",
} as const;
...
export const INCLUDE_COMBO_ODDS_BATCH_NOTE =
  "設定でワイド・馬連・三連複のオッズ取得をONにしています。取得したオッズを馬券配分に使うかどうかは「ワイドを配分に使う」「馬連を配分に使う」「三連複を配分に使う」の設定に従います。";
...
export const ALLOCATION_BET_TYPE_LABELS = {
  wide: {
    ...
    /** チェックボックス直下の補助文。 */
    help:
      "既定でONです。上記「ワイド・馬連・三連複のオッズも取得する」がOFFの間は効果がありません(取得したオッズが無いため)。ONにすると、複勝の提案額が変わることがあります(理由は一括分析画面の配分内訳に表示します)。",
  },
  /**
