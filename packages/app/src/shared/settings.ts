 ...
 * 分割〈2026-09-26〉より前に書かれたもので、分割後の実際の担当タスクとは#123ではなく
...
export const INCLUDE_COMBO_ODDS_LABELS = {
  ...
  /** チェックボックス直下の補助文。 */
  help:
    "取得に時間がかかります(地方競馬の三連複は1レースあたり最大16リクエスト・約24秒。馬連・馬単はそれぞれ1レースあたり常に1リクエスト追加。12レース一括で+約5分)。取得したオッズを馬券配分に使うかどうかは、下記「ワイドを配分に使う」「馬連を配分に使う」「三連複を配分に使う」の設定に従います。馬単は現在は取得のみで配分には使いません。",
} as const;
 ...
 *
 * **Issue #122で馬単も明記した**(INCLUDE_COMBO_ODDS_LABELS.helpと同じ理由・同じ判断:
 * 配分利用の列挙〈3項目〉には加えず、末尾に「馬単は現在は取得のみで配分には使わない」旨を
 * 添える)。
 */
export const INCLUDE_COMBO_ODDS_BATCH_NOTE =
  "設定でワイド・馬連・馬単・三連複のオッズ取得をONにしています。取得したオッズを馬券配分に使うかどうかは「ワイドを配分に使う」「馬連を配分に使う」「三連複を配分に使う」の設定に従います。馬単は現在は取得のみで配分には使いません。";
