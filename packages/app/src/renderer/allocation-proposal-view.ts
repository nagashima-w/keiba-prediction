 ...
 *
 * **②③は特定の券種名を列挙しない(メタレビュー差し戻し・Issue #117)。** これらの文言は
 * 過去に保存された分析(`analysis_allocation_meta`)を再表示するときにも使われる。
 * 券種の対応状況は時期によって異なる(例: 馬連はIssue #117より前は配分の候補ビルダーが
 * 存在せず、`includeQuinellaInAllocation`という設定自体が無かった時期もある)ため、
 * 「ワイド・馬連・三連複」のように現在の対応券種を列挙すると、その券種がまだ存在しなかった
 * 時期の記録に対しても同じ文言が出て、実際には評価されていない券種を判定結果があったかの
 * ように見せてしまう(#31「判定済みを未判定に潰さない」の逆方向の違反: ここでは
 * 「判定していないものを判定済みのように見せる」)。将来また新しい組合せ券種が増えても
 * この文言を書き換える必要が無いよう、「組合せ券種」という総称のみを使う。
 */
