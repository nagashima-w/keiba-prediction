 ...
 *
 * 実測(中央16頭・候補406件)で1レースあたり約93msかかり、`BatchAnalysisView.tsx` は
 * render 内 IIFE で毎レンダー全レース分を評価するため、details の開閉・Discord送信状態の
 * 変化などあらゆる再レンダーで再計算が走ると12レースで約1.1秒/レンダーの体感の重さになる
 * (性能上の理由)。**しかしそれ以上に危険なのは、キャッシュキーに含める入力を1つでも
