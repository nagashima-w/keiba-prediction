 ...
 *    #26-P3で"2026-07-22.1"に更新)。
 *  - 新版(wide15)は幅0.15(絶対値)・対照と同じ日付系列+"-clip015"接尾辞の版文字列
 *    (ユーザー確定事項A: 対照のPROMPT_VERSION更新に新版も追随する運用。#26-P3で"2026-07-22.1-clip015"に更新)。
...
describe("CLIP_VARIANTS(クリップ幅の版registry)", () => {
  it("対照(default)は幅0.10・版文字列が既存PROMPT_VERSIONと同一の2026-07-22.1であること(#26-P3で追随)", () => {
    expect(CLIP_VARIANTS.default.id).toBe("default");
    expect(CLIP_VARIANTS.default.maxAdjust).toBe(0.1);
    expect(CLIP_VARIANTS.default.promptVersion).toBe("2026-07-22.1");
  });
  ...
  it("新版(wide15)の版文字列は「対照と同じ日付系列」+「-clip015」接尾辞であること(#26-P3で対照に追随。ユーザー確定事項A)", () => {
    // 対照が"2026-07-22.1"に更新されたら、新版は"2026-07-22.1-clip015"(対照の値+接尾辞)になる。
    expect(CLIP_VARIANTS.wide15.promptVersion).toBe(
      `${CLIP_VARIANTS.default.promptVersion}-clip015`,
    );
    expect(CLIP_VARIANTS.wide15.promptVersion).toBe("2026-07-22.1-clip015");
  });
