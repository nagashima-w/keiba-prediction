 ...
 * (2)人気と複勝配当: 複勝圏内馬の人気レンジ・二桁人気の頭数・複勝配当のレンジ/中央値、
 ...
 * CLIP_VARIANTS.wide15.promptVersion も同じ値+"-clip015"へ追随する(ユーザー確定事項A)。
 *
 * "2026-07-28.2"(機能Bの小改善・プロンプト誤読解消。2026-07-28 boss着手前ゲート合意): 2行目
 * (人気と複勝配当)の見出し「複勝圏内(N頭)」は、Nが3着以内の延べ頭数であって、後続の人気・
 * 複勝配当それぞれの標本数ではないのに、文の係り方から「N頭ぶんの人気・N頭ぶんの配当」と
 * 誤読できた(標本数が食い違う実在条件がある: fuku_pay3欠損・複勝非発売・3着同着・payback丸ごと
 * null・fuku_payが0や非有限・複勝圏内馬のninkiがnull/0等)。見出しを「複勝圏内(延べN頭)」に
 * 明確化し、人気・複勝配当それぞれの行に自身のサンプル数(n=)を併記する形へ変更した
 * (「人気1〜9番人気(n=24、うち二桁人気0頭)、複勝配当200〜430円(中央値280円、n=24)」)。
 * 集計ロジック自体(grade-winner-trend.ts。何を数えるか)は変更しておらず、既存5集計値の数値も
 ...
 *
 * 2行目の見出し「複勝圏内(延べN頭)」について(2026-07-28小改善): Nは3着以内の延べ頭数であり、
 * 後続の人気・複勝配当それぞれの標本数とは別物(fuku_pay3欠損・複勝非発売・3着同着・payback丸ごと
 * null・fuku_payが0や非有限・ninkiがnull/0等で食い違う実在条件がある)。誤読を避けるため、
 * 「延べ」を明記し、人気・複勝配当それぞれに自身のサンプル数(n=)を併記する
 * (例:「人気1〜9番人気(n=24、うち二桁人気0頭)、複勝配当200〜430円(中央値280円、n=24)」)。
 */
function gradeWinnerTrendLines(
  ...
  // 2行目: 人気と複勝配当(複勝圏内馬の人気レンジ・二桁人気の頭数・複勝配当のレンジ/中央値)。
  // 見出しのNは3着以内の延べ頭数であり、人気・複勝配当それぞれの標本数(n=)とは別物のため、
  // 「延べ」を明記し各値に自身のサンプル数を併記する(誤読解消。2026-07-28小改善)。
  const marketParts = [
    summary.人気レンジ
      ? `人気${summary.人気レンジ.min}〜${summary.人気レンジ.max}番人気(n=${summary.人気サンプル数}、うち二桁人気${summary.二桁人気頭数}頭)`
      : null,
    summary.複勝配当レンジ && summary.複勝配当中央値 !== null
      ? `複勝配当${summary.複勝配当レンジ.min}〜${summary.複勝配当レンジ.max}円(中央値${summary.複勝配当中央値}円、n=${summary.複勝配当サンプル数})`
      : null,
  ].filter((x): x is string => x !== null);
  if (marketParts.length > 0) {
    lines.push(`複勝圏内(延べ${summary.複勝圏内馬数}頭)の傾向: ${marketParts.join("、")}`);
  }
