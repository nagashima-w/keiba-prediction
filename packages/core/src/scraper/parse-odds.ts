 ...
 *
 * レスポンスは `{status:"result", data:{official_datetime, odds:{"1":単勝, "2":複勝}}}`。
...
export function parseOdds(json: string): OddsSnapshot {
  ...
  if (parsed.status !== "result") {
    throw new OddsParseError(
      `status が "result" ではありません(status: ${JSON.stringify(parsed.status)})`,
    );
  }
  ...
  if (winRaw === undefined || placeRaw === undefined) {
    throw new OddsParseError(
      "単勝(odds[1])または複勝(odds[2])が欠落しています",
    );
  }
  ...
  return { officialDatetime, win, place };
}
