 ...
 *
 * ★馬単(exacta)はIssue #121・#24-F2で確定払戻の取込・回収率判定に対応したが、配分が馬単の
 * 買い目をまだ作らない(#123・#24-E3bで対応予定)ため、`betCount`は現時点で構造的に常に0になる。
...
export interface ProposedBetReport {
   ...
   * 現時点で常に0(このJSDoc冒頭の注意参照)。画面(`VerifyView.tsx`)への表示は#123で行う。
   */
