 ...
 * **旧版(#78着手前ゲート第3回)の「上限200では最悪3.56ms・非収束14.5%」という記述は、
 * この表の200行(最悪13.8ms/9.3ms・非収束3.09%/3.00%)と食い違う。** 旧版は本表と異なり
 * 頭数18固定・N=200(`runModelLayer`と同じ生成方法)で計測したものであり、本表(頭数を
 * 振ってN=3200まで拡張)とは母集団が異なる。数値は必ず本表(またはその再実測)を参照し、
 * 旧版の単一の数字を転記しない。
 *
