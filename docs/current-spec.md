...
本書は **実際に実装されている現状(v1.1.0)** をまとめたもの。当初の設計・計画は
...
- バージョン: ルート/アプリ `1.1.0`、`@keiba/core` `0.2.0`
...
- **券種横断の配分(機能D-2c・Issue #28)**: 複勝・ワイド・三連複を**同じ1つの予算枠**の中で
  ...
  状態で判別する)を含む。**組合せ券種のEVは推定誤差が組み合わせ人数ぶん増幅されて過大評価になりやすく、
...
- **レビュー継続中コミットの扱い**: コミットメッセージに「レビュー継続中」を含む push では、
  未承認コードが `dev-latest` に公開されるのを防ぐため、公開・孤児掃除の両ステップをスキップする
