...
本書は **実際に実装されている現状(v1.9.8)** をまとめたもの。当初の設計・計画は
...
- **対応券種は用途によって異なる(3点。混同しないこと)**:
  ...
  - **記録・回収率検証**: 新方式(proposedBet系、`ev/verify.ts`の`computeProposedBetReport`。
    ...
    `analysis_bets`テーブルへ`bet_type`列付きで保存された買い目のうち、この5券種以外の
...
- **券種横断の配分(機能D-2c・Issue #28)**: 複勝・ワイド・三連複を**同じ1つの予算枠**の中で
  ...
  **v1.9.8 時点では券種の選択〈`resolveMixedBetTypes`〉はこの値を読まず、設定画面にも出ない**
  (配分提案が馬連を扱うのは #24-D3b から)。
