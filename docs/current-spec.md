...
本書は **実際に実装されている現状(v1.10.6)** をまとめたもの。当初の設計・計画は
...
- **対応券種は用途によって異なる(3点。混同しないこと)**:
  ...
  - **記録・回収率検証**: 新方式(proposedBet系、`ev/verify.ts`の`computeProposedBetReport`。
    Issue #71・#54-B)は複勝・単勝・ワイド・三連複・馬連・馬単の6券種に対応(単勝はIssue #100・#23-Cで、
    馬連はIssue #114・#24-F1で、馬単はIssue #121・#24-F2で追加。それ以前はそれぞれが「未対応の券種コード」扱いだった)。
    `analysis_bets`テーブルへ`bet_type`列付きで保存された買い目のうち、この6券種以外の
  ...
  - **馬連・馬単ともに配分提案・画面表示まで含めて対応済み**(#24-D3シリーズ・#24-Eシリーズ)。
    ...
    **馬単対応の経緯**(馬連〈#112〜#118〉と同じ切り方。`#24-E`は`#120`/`#121`/`#122`/`#123`に
    ...
    切り方。下記「配分記録の永続化」節参照)
- バージョン: ルート/アプリ `1.10.6`、`@keiba/core` `0.2.0`(`@keiba/core` は版数運用の対象外・据え置き。
...
- **配分ベースの回収率(proposedBet系、Issue #71・#54-B)**: `VerifyReport.proposedBet`
  ...
  `proposedBet` 内部の複勝・単勝・ワイド・三連複・馬連・馬単の6券種(単勝はIssue #100・#23-Cで、
  馬連はIssue #114・#24-F1で、馬単はIssue #121・#24-F2で追加)は同一の賭け金仮定(実際の配分額)を共有するポートフォリオの
  ため、`overall`(6券種の合算)は持つ。**馬連はIssue #117(#24-D3b-2)、馬単はIssue #125
