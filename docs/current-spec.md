...
本書は **実際に実装されている現状(v1.9.6)** をまとめたもの。当初の設計・計画は
...
- **対応券種は用途によって異なる(3点。混同しないこと)**:
  ...
  - 馬連・馬単・三連単・枠連・枠単は未対応(拡張のロードマップと技術的な依存関係は GitHub Issue #22)。
    ただし**馬連は core の確率・候補ビルダー・配分だけ先に対応済み**(Issue #112・v1.9.5。`buildQuinellaCandidates` が
    順序付き outcome 空間から1着・2着の集合を周辺化して的中確率を求める。上位3着の集合から求めるとワイドと同じ値に
    なるため)。**馬連オッズのパーサ・取得関数も対応済み**(Issue #113・v1.9.6。中央 `api_get_jra_odds` の `type=4`・
    地方 `odds/index.html?type=b4`。キーは昇順正規化・値は単一値)。ただし `scrape-race.ts` には配線しておらず
    実際の取得は行わないので、利用者から見える変化はまだ無い(#24-D3)
- バージョン: ルート/アプリ `1.9.6`、`@keiba/core` `0.2.0`(`@keiba/core` は版数運用の対象外・据え置き。
...
- **配分計算のタイミング(Issue #110・v1.9.4)**: 一括分析画面の券種横断の配分は、**描画の中で同期的に
  ...
  ただし1レースの計算中は画面が止まる(止まる時間がレース1つ分に縮むだけ。完全に止まらなくするには
  別スレッド化が要る)。
