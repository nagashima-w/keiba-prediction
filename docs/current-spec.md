...
本書は **実際に実装されている現状(v1.6.6)** をまとめたもの。当初の設計・計画は
...
- バージョン: ルート/アプリ `1.6.6`、`@keiba/core` `0.2.0`(`@keiba/core` は版数運用の対象外・据え置き。
...
- **同時分布モデル**(`place-joint-model.ts`): 「どの馬の組合せが複勝圏内に入るか」の同時分布を、
  既定では条件付きベルヌーイ分布 `P(S)=Πwᵢ/ΣΠwᵢ`(`wᵢ=pᵢ/(1−pᵢ)`)で近似する
  (`CONDITIONAL_BERNOULLI_MODEL`)。周辺確率の合計は「ちょうど k 頭が複勝圏内」という
  条件付けにより **k(複勝の対象人数)へ正規化される**。`PlaceJointModel` インタフェースを
  差し替え点として用意しており、Issue #77(#20-A)で厳密なモデル(`PLACKETT_LUCE_MODEL`。
  潜在強度 θ を推定し Plackett-Luce の上位k集合分布を厳密に構築する)を追加した。
  **ただし既定モデルはまだ `CONDITIONAL_BERNOULLI_MODEL` のまま**で、`bet-allocation.ts`・
  `combo-bet-allocation.ts` はこれを切り替えていない(既定切替は #78〈#20-B〉のスコープ)。
  ...
  予測精度の指標ではない)。UI で近似かどうかを出し分ける画面は無い(値は結果に載り DB にも
  保存されるが、表示を出し分ける消費者はまだ存在しない)。
...
- **Issue #35 の分割**: #40(本節。計測基盤の健全化と指標の実装)/ #39(本番側
  ...
  #42(較正 calibration 方式の要否検討)。同時分布モデルの厳密化(#20)は #41/#42 の技術的前提であり、
  #77(#20-A。θ推定器と`PLACKETT_LUCE_MODEL`の追加・既定は不変)→ #78(#20-B。既定モデルの切替・
  エンジン側の出力検証・期待値の一括更新)→ #79(#20-C。`probability-quality-metrics.ts` の
  2関数の切替可否)の3段に分割して進めている(#77 完了時点)。
