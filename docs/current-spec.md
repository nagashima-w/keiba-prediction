...
本書は **実際に実装されている現状(v1.9.5)** をまとめたもの。当初の設計・計画は
...
- **対応券種は用途によって異なる(3点。混同しないこと)**:
  ...
  - 馬連・馬単・三連単・枠連・枠単は未対応(拡張のロードマップと技術的な依存関係は GitHub Issue #22)。
    ...
    なるため)。オッズの取得と画面への配線は未実装なので、利用者から見える変化はまだ無い(#24-D2 / #24-D3)
- バージョン: ルート/アプリ `1.9.5`、`@keiba/core` `0.2.0`(`@keiba/core` は版数運用の対象外・据え置き。
