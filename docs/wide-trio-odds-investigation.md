...
## 12. jiku=1フラグメントの実測(機能D-2b-B・Issue #33 第1段・AC6、2026-08-07)

#33の設計(軸1〜n−2を機械的に走査して全組合せC(n,3)を網羅する)は、`jiku=k`が「kを含む
全トリオを返す」という#13・#32での実測(`jiku=2`のみ検証済み)の上に成り立っている。しかし
軸1(走査の起点)自体の応答が通常ページ(`odds/index.html?type=b7`、軸1固定で表示される)と
一致するかは#32時点でも未検証だった。#33着手前ゲート(boss)の指示により、これを実装より
先に反証する(鉄則6)。

### 12.1 実リクエスト(1件。上限2件のうち1件を使用)

`scripts/fetch-nar-trio-axis-fixture.ts`(コミット済み、`HttpClient`経由・最低1.5秒間隔・
CIから呼ばない)で以下を取得した。

| 対象 | URL | 取得日時(UTC) | 保存先 |
|---|---|---|---|
| 地方・高知10R ファイナルレース(race_id=202654071210、12頭)の3連複、軸1 | `https://nar.netkeiba.com/odds/odds_get_form.html?type=b7&race_id=202654071210&jiku=1` | 2026-08-07T09:47:50.593Z | `fixtures/nar_odds_b7_jiku1_202654071210.html` |

`fetchText`の戻り値を得た直後に`writeFileSync`で保存し、解析はその後に行った(AC10の
「構造的に保存が先」)。

### 12.2 検証結果: 一致(前提は崩れなかった)

`parseNarComboOdds`(既存・変更なし)で`nar_odds_b7_jiku1_202654071210.html`と
`nar_odds_b7_202654071210.html`(通常ページ)をそれぞれパースし突合した
(`packages/core/test/scraper/parse-nar-combo-odds.test.ts`
「jiku=1フラグメント不変条件」describeで固定):

- **キー集合**: 両方とも55件、`Set`として完全一致
- **値**: 55件全件で`oddsMin`/`oddsMax`/`ninki`が一致(不一致0件)

独立検証として、vitestとは別にPythonの正規表現抽出で同じ2ファイルを突合し、同じ結論
(55件・キー集合一致・値差分0件)を確認した(交絡変数を減らすため、本番パーサ経由の検証と
独立した経路で二重チェックした)。

**結論**: `jiku=1`のAJAXフラグメントは通常ページ(軸1固定表示)と組合せ集合・値の両方が
完全に一致する。#33の軸集合走査設計(軸1〜n−2をAJAXフラグメント経由で叩く)の前提は
崩れなかったため、着手前ゲートで合意した設計のまま第2段(`narTrioOddsAxisUrl`+軸集合導出)
に進める。
